{ **************************************************************************** }
{                                                                              }
{ URL: https://stackoverflow.com/questions/5639531/                            }
...
unit BufferedFileStreamUnit;

interface

uses
  System.SysUtils, System.Math, System.Classes, Winapi.Windows;
...
type
  ...
  *)
  ...
  protected
    function CreateHandle(FlagsAndAttributes: DWORD): THandle; override;
    function GetFileSize: Int64; override;
    procedure SetSize(const NewSize: Int64); override; //Vitaly 20.11.2017
  public
    ...
    constructor Create(const FileName: string; Append: Boolean;
      CacheSize: Integer = 0; ReadStreamCacheSize: Integer = 0;
      ReadStreamUseAlignedCache: Boolean = False); overload;
    destructor Destroy; override;
    function Read(var Buffer; Count: Longint): Longint; override;
    function Write(const Buffer; Count: Longint): Longint; override;
  end;
...
begin
  ...
  if not SetEndOfFile(FHandle) then begin
...
begin
  if ReadFile(FHandle, Buffer, Count, LongWord(Result), nil) then begin
    FLastError := 0;
  end else begin
...
begin
  if WriteFile(FHandle, Buffer, Count, LongWord(Result), nil) then begin
    FLastError := 0;
  end else begin
    FLastError := GetLastError;
    Result := -1;
...
begin
  Create(FileName, False, CacheSize, ReadStreamCacheSize, ReadStreamUseAlignedCache);
end;
...
constructor TWriteCachedFileStream.Create(const FileName: string;
  Append: Boolean; CacheSize, ReadStreamCacheSize: Integer;
...
begin
  ...
  Result := CreateFile(
    ...
    FILE_SHARE_READ, //Vitaly 20.11.2017
    nil,
...
procedure TWriteCachedFileStream.SetSize(const NewSize: Int64);
begin
  //Vitaly 20.11.2017
  FlushCache;
  FPosition := FileSeek(NewSize, soBeginning);
  FFileSize := FPosition;
  FCacheStart := FPosition;
  FCacheEnd := FPosition;
  if not SetEndOfFile(FHandle) then begin
